...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 处理实现Aware一系列接口中的参数注入
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));

		// 设置忽略自动装配的接口，即ApplicationContextAwareProcessor处理注册的那些Aware相关实现
		...
		// 将用于检测内部bean的早期post-processor注册到ApplicationListeners。
		beanFactory.addBeanPostProcessor(new ApplicationListenerDetector(this));

		// Detect a LoadTimeWeaver and prepare for weaving, if found.
		// 如果当前BeanFactory包含loadTimeWeaver Bean，说明存在类加载期织入AspectJ（静态代理），
	 ...
	 * 所有beanDefinition都应该是已加载，但还没有实例化bean。
	 ...
	 * 如果在当前上下文中找不到则使用SimpleApplicationEventMulticaster
	 *
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
