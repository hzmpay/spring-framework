...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	 ...
	 * 是否允许使用相同的名称重新注册不同的beanDefinition
	 */
	 ...
	 * a different definition with the same name, automatically replacing the former.
	 *
	 * <br/>allowBeanDefinitionOverriding = 是否允许使用相同的名称重新注册不同的beanDefinition，
	 * 如果可以的话则覆盖原来的beanDefinition
	 *
	 * @since 4.1.2
	 */
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		// beanName在beanDefinitionMap注册过
		if (existingDefinition != null) {
			// 是否允许使用同一个名称去重新注册另一个不同的beanDefinition
			if (!isAllowBeanDefinitionOverriding()) {
				throw new BeanDefinitionOverrideException(beanName, beanDefinition, existingDefinition);
			}
			// 可以重复注册，进行一些比如bean重载之类的日志输出
			else if (existingDefinition.getRole() < beanDefinition.getRole()) {
				// e.g. was ROLE_APPLICATION, now overriding with ROLE_SUPPORT or ROLE_INFRASTRUCTURE
				// 例如，以前是ROLE_APPLICATION，现在是ROLE_SUPPORT或ROLE_INFRASTRUCTURE
		...
		// beanName在beanDefinitionMap没注册过，进行注册
		else {
			// 检查这个BeanFactory是否存在注册中或者注册完成的bean
			if (hasBeanCreationStarted()) {
				// BeanFactory中存在注册中或者注册完成的bean
		...
		// 这个beanName已经注册过 or 这个beanName已经存在与单例对象缓存中
	...
	protected void resetBeanDefinition(String beanName) {
		...
		// 如果有的话，从单例缓存中删除相应的bean。
		// 通常不应该是必要的，而不仅仅是用于覆盖上下文的默认bean
		// (例如StaticApplicationContext中的默认StaticMessageSource)。
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			// 解析各种类型的值进行匹配，如：Map，List，Stream等
